...
mainApp.controller('ImageController', function($scope, $cordovaDevice, $cordovaFile, $ionicPlatform, $cordovaEmailComposer, $ionicActionSheet, ImageService, FileService,RestService) {
  ...
  $scope.addImage = function(type) {
    $scope.hideSheet();
    ImageService.handleMediaDialog(type).then(function(newName) {
        
        
        
        RestService.uploadMedia("http://localhost:3000/dummyService/saveNewImage",$scope.urlForImage(newName)).then(function(data){
        }).then(function(erro){}).then(function(progress){});
            $scope.images = FileService.images();
    });
  }
